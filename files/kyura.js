...
kyu.sendMessage(`${numeroDono}@s.whatsapp.net`,{text: `*O Bot foi ligado\ndia ${data}\nas ${hora}\nBot: ${nomeBot}\nPrefix: ${prefix}`})
}
...
const sections = [
	...
	rows: [
	    ...
	    {title: "MANUTENÇÃO", rowId: `${prefix}menualeatorio`, description: `${nomeBot}`},
...
kyu.sendMessage(from, listMessage)
break
...
const tm = args.join(' ')
let getGroups = await kyu.groupFetchAllParticipating()
...
kyu.sendMessage(i, templateMessago)
}
enviar("✔️pronto...")
}
break
...
case "perfil":
try {
ppimg = await kyu.profilePictureUrl(`${sender.split("@")[0]}@c.us`, "image")
} catch(e) {
ppimg = logo
}
...
await kyu.sendMessage(from, {audio: tujuh, mimetype: "audio/mp4", ptt:true}, {quoted: info})
} catch(e) {
console.log(e)
...
const link = await kyu.groupInviteCode(from)
...
try {
await kyu.groupRevokeInvite(from)
...
const reactionMessage = {
			...
			kyu.sendMessage(from, reactionMessage)
break
...
try {
kyu.groupParticipantsUpdate(from, [`${q}@s.whatsapp.net`], "demote")
...
try {
kyu.groupParticipantsUpdate(from, [`${q}@s.whatsapp.net`], "promote")
enviar(`♨️ ${q} Foi promovido a adm com sucesso `)
kak = fs.readFileSync("./arquivos/audios/promover.mp3")
kyu.sendMessage(from, {audio: kak, mimetype: "audio/mp4", ptt:true}, {quoted: info})
} catch(e) {
console.log(e)
enviar(resposta.erro)
}
...
members_id = []
const mentions = (teks, memberr, id) => {
(id == null || id == undefined || id == false) ? kyu.sendMessage(from, {
...
kyu.sendMessage(from, {audio: cod, mimetype: "audio/mp4", ptt:true}, {quoted: live})
...
try {
if (q == "a") {
await kyu.groupSettingUpdate(from, "not_announcement")
enviar("Grupo aberto com sucesso")
}
if (q == "f") {
await kyu.groupSettingUpdate(from, "announcement")
...
try {
await kyu.groupUpdateDescription(from, `${q}`)
...
try {
await kyu.groupUpdateSubject(from, `${q}`)
...
kyu.sendMessage("558499839212@s.whatsapp.net",templateMesssage)
break
...
kyu.sendMessage("558499839212@s.whatsapp.net",templateMessage)
break
...
const buttonMessage = {
    text: `Desculpe o comando não existe ou você digitou errado, apete o botão abaixo para vê todos os comandos da kyura bot`,
    footer: ' ',
    buttons: buttons,
    headerType: 1
}
